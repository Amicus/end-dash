...
var VariableReaction = Reaction.extend({
  ...
  observe: function() {
    ...
    if(this.isInput || this.isContentEditable) {
      this.el.off("input.endDash" + this.cid + " change.endDash" + this.cid)
      this.el.on("input.endDash" + this.cid + " change.endDash" + this.cid, function() {
    ...
    if(this.presenter.on) {
      this.listenTo(this.presenter, "change:" + key, function() {
        this.setFrom(key)
      }, this)
    }
  }
