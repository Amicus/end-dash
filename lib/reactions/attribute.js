...
var AttributeReaction = Reaction.extend({
   ...
   **/
  observe: function() {
    var attrString = _.values(this.attributes),
        splitAttrString = attrString.join(" ")
                                    .split(/(#{[^}]*})/),
        interpolatedProperties = _.chain(splitAttrString)
                                  .select(function(str){ return str.match(/#{/) })
                                  .map(function(str){ return str.substr(2, str.indexOf("?") != -1 && str.indexOf("?") || str.length - 3) });
    listenHash = interpolatedProperties.value()
    if (_.isArray(listenHash)) {
      listenHash = listenHash.join(" ")
    }
    this.change(listenHash, function() {
      this.updateAttributes(this.attributes);
    }, this);
  }
