...
var DeprecatedLoopingReaction = Reaction.extend({
  ...
  getTemplateClass: function(model) {
    ...
    if (this.polymorphicKey)
      template = Template.extend(this.itemTemplates[get(model, this.polymorphicKey)]);
    if (!template)
      template = Template.extend(this.itemTemplates["default"]);
    return template
  },
  ...
  parse: function(el, state) {
    ...
    children.each(function(i, element) {
      ...
      templates[whenValue] = (new Parser(child, {
        templateName: _.last(state.pathStack), //same as its parent
      })).export() 
      child.remove();
    });
