...
var LoopingReaction = Reaction.extend({
  ...
  init: function(next) {
    ...
    if(!this.collection)
      throw new Error("could not find collection " + this.collectionName);
...
//    }
  ...
  parse: function(el, state) {
    var template,
        templateName = _.last(state.pathStack) + 'default';
    EndDash.registerTemplate(templateName, el);
    template = EndDash.getTemplateClass(templateName);
