...
Template.prototype.freeze = function() {
  var structure = mapTraverse(this.structure, function(structure) {
    ...
    if(structure.Reaction) {
      frozen.properties = this.reactions[structure.id].freeze()
...
Template.prototype.traverse = function(structure, stack, callback) {
  ...
  function next(stack) {
    _(structure.children).each(function(child) {
      that.traverse(child, stack, callback)
    })
  }
  if(!structure.Reaction) {
    return next(stack)
