...
Parser.loadAndParse = function(templatePath, markup) {
  TemplateStore.load(templatePath, markup);
  var parserRun = new Parser(markup, {
                    templateName: TemplateStore.templateNameForPath(templatePath),
                    templates: TemplateStore.raw_templates()
                  })
      , templateClass = parserRun.generate()
  TemplateStore.loadParsed(templatePath, templateClass);
  return templateClass;
};

Parser.getTemplateClass = function(templatePath) {
  if (TemplateStore.raw_templates(templatePath)) {
    if (!TemplateStore.isParsed(templatePath)) {
      return this.loadAndParse(templatePath, TemplateStore.getLoaded(templatePath));
    }
    return TemplateStore.getParsed(templatePath);
  } else {
    throw new Error('Could not find template '+opts.templateName)
  }
};

Parser.load = TemplateStore.load;
