    ...
    if(el.is(selector)) {
      _(reactions).each(function(Reaction) {
        ...
        if(reaction.reactIf(el)) {
          that.handleReaction(el, reaction)
...
Parser.prototype.handleAfterReaction = function(el, reaction) {
  this.templateProto.structure.push({ 
...
Parser.prototype.handleReaction = function(el, reaction) {
  if(reaction.parse) {
    reaction.parse(el)
  }
  this.templateProto.structure.push({ 
...
Parser.prototype.afterReactions = function(el, toClose) {
  _(toClose).each(function(reaction) {
    this.handleAfterReaction(el, reaction)
  }, this)
}
