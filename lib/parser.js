...
var Parser = module.exports = function(markup, opts) {
  ...
  if (rootElements(markup).length > 1) {
...
var _htmlFromNode = function(node) {
  return $($('<body/>').append(node)).html()
}

Parser.rootElements = function(markup) {
  nodes = $.parseHTML( markup );
  clean_nodes = _.reject(nodes, function(node) { return !$.trim(_htmlFromNode(node)) })
  return _.map(clean_nodes, function(node) { return _htmlFromNode(node) });
}
