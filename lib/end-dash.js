  ...
  this.traverse(function(name, type, element) {
    if(type == "variable" || type == "conditional") {
      ...
      var Factory = this.collectionFactories[name]
        ...
        , collectionModel = this.getFrom(name, data)

      if(!collectionModel) {
        throw new Error("Unable to find associated collection: " + name)
      }
      ...
      var Factory = this.subTemplateFactories[name]
        ...
        , templateModel = this.getFrom(name, data || {})
      ...
      if(!templateModel) {
        throw new Error("Unable to find associated model: " + name)
      }
    }
  })
}
...
function findDescendantsAndSelf(element, selector) {
  ...
  matches = matches.add(element.filter(selector))
  return matches
}
