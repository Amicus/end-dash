...
function TemplateFactory(markup, opts) {
  ...
  //parse unless the user didn't EXPLICITLY tell the factory not to parse the markup if(opts.parse !== false)
  if(opts.parse !== false) {
    this.parse()
  }
}
...
TemplateFactory.prototype.parse = function() {
  var that = this
  this.parser = new Parser(this.markup, { proto: this.templateProto, templates: templateRegistry, rootTemplate: this.name, reactions: reactions })
