...
TemplateFactory.prototype.mapCollection = function(name, element) {
  ...
  this.templateProto.structure.push({ key: key, selector: selector, type: "collection" })
...
TemplateMethods.bind = function() {
  var modelStack = [ data ]
    , viewStack = []

  this.traverse(function(name, type) {
      if(type == "variable") {
        this.set(name), _.(modelStack).last().get(name))
        _.(modelStack).last().on("change:" + name, function() {
          var template = that
            , model = this
          template.set(name), model.get(name))
        })
      } else if(type == "collection") {
        var Factory = this.collectionFactories[name]
          , CollectionTemplate = Factory.generate(this.template)
          , collectionModel = _.(modelStack).last().get(name)

        this.collections[name] = new Collection(collectionModel)
...
TemplateMethods.traverse = function(iterator) {
  this.structure.each(function(node) {
    iterator(node.key, node.type)
  })
}
