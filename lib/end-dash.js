...
TemplateFactory.prototype.mapElement = function(key, element, name, value) {
  var type = (name == "innerHTML") ? "variable" : "attribute"
  ...
  this.templateProto.structure.push({ key: key, selector: selector, type: type })
...
TemplateFactory.reactTo = function(selector, events) {
  reactions[selector] = reactions[selector] || []
  reactions[selector].push(events)
}


TemplateFactory.reactTo('[data-view], [class$="View-"], [class*="View- "]', {
...
TemplateFactory.prototype.mapConditional = function(key, element) {
  ...
  if(negation = key.match(/not(.+)/)) {
    key = negation[1].charAt(0).toLowerCase() + negation[1].slice(1)
    negation = true
  } else {
    negation = false
  }
...
TemplateFactory.prototype.mapReaction = function(el, events) {
  function Reaction() {
    this.state = {}
  }
  Reaction.prototype = events
...
TemplateFactory.prototype.parse = function() {
  ...
  this.parser.on("reaction", function(el, events) {
    _(stack).last().mapReaction(el, events)
  })
...
TemplateMethods._readFrom = function(name, presenter) {
  var that = this

  this.setFrom(name, presenter)
  if(typeof presenter.on === "function") {
    presenter.on("change:" + name, function() {
      that.set(name, presenter.get(name))
    }, this)
  }
}

TemplateMethods._writeTo = function(element, name, model) {
  var that = this
  
  if(typeof model.set === "function") {
    if(this.handler) {
      element.off("input", this.handler)
    ...
    this.handler = function() {
      ...
      if(el.is(":input")) {
        ...
        model.set(name, $(this).html())
      }
    }
    //need to register both for select elements to work
    element.on("input", this.handler)
...
TemplateMethods.bind = function(data) {
  ...
  this.traverse(function(name, type, element, options) {
    ...
    if(type == "variable" || type == "conditional" || type === "attribute") {
      this._readFrom(name, presenter)
      if(type === "variable" && element.is(":input:not(button), [contenteditable]")) {
        this._writeTo(element, name, model)
      }
    } else if(type === "collection") {
      this.bindCollection(name, element, data)
    } else if(type === "model") {
      this.bindModel(name, element, presenter)
    } else if(type === "reaction") {
      if(options.reaction.init) {
        options.reaction.init(element, model)
      } 
      if(options.reaction.afterDOMConstruction) {
        afterDOMConstruction.push(function() { options.reaction.afterDOMConstruction(element, model, that) })
      }
    }
  })
...
TemplateMethods.set = function(key, value) {
  ...
  if(keyConfigs = that.keys[key]) {
    _(keyConfigs).each(function(keyConfig) {
      //if there is no selector we need to hide the container
      if(that.container && !keyConfig.selector) {
        var element = that.container
      } else {
        var element = findDescendantsAndSelf(that.template, keyConfig.selector)
      }
      if(keyConfig.type === "variable") {
...
TemplateMethods.setAttribute = function(element, attributeName, attributeValue, replaceKey, replaceValue, valueIfTrue, valueIfFalse) {
  ...
  } else if(replaceValue === true) { //boolean true, just use the name, CONVENIENT for class names
    replaceValue = replaceKey
  }
  if(valueIfFalse) {
    regex += "\\:\\s*" + valueIfFalse + "\\s*"
  } else if(!replaceValue) { //boolean false, use the empty string
    replaceValue = ""
  }                                    
  regex += "}"
  //if a boolean true, use it's name
