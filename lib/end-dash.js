...
TemplateFactory.prototype.mapElement = function(key, element, name, value) {
  ...
  if(type === "attribute") {
    var match = value.match(/([a-zA-Z ]+)(?:\?([a-zA-Z ]*))?/)
      , valueIfTrue
  ...
  if(keyConfigs = that.keys[key]) {
    _(keyConfigs).each(function(keyConfig) {
      ...
      if(keyConfig.type === "innerHTML") {
        ...
        that.setAttribute(element, keyConfig.attributeName, keyConfig.attributeValue, key, value, keyConfig.valueIfTrue)
      }
    })
  }
}
...
TemplateMethods.setAttribute = function(element, attributeName, attributeValue, replaceKey, replaceValue, valueIfTrue) {
  if(valueIfTrue) {
    replaceValue = (replaceValue === false) ? "": valueIfTrue
  }
