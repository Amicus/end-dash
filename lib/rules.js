...
window.syntaxWarningOccurred = false
...
rules.deprecatedIterationCheck = function(el) { //Deprecated
  if (this.collection(el) || this.collection(el.parent())) {
    if (!syntaxWarningOccurred) {
      console.log("***********************WARNING***********************\n" +
                  "EndDash here, you are using a depcrecated iteration syntax. Some day I might change." +
                  "and then well, your code just won't work")
    }
    syntaxWarningOccurred = true
    return true
  }
  return false
}
