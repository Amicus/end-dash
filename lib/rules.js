...
rules.deprecatedIterationCheck = function(el) { //Deprecated
  iterationName = this.deprecatedCollection(el) || this.deprecatedCollection(el.parent())
  if (iterationName) {
    //console.log("****ENDDASH-DEPRECATED-ITERATION-SYNTAX-WARNING****: collection:" + iterationName)
    return true
  }
  return false
}
...
rules.variable = function(el) {
  ...
  if (this.polymorphicKeyClass(variableName) || this.polymorphicValueClass(variableName)) {
    return false
  }
