...
describe("Setting a single variable with scoping", function() {
  beforeEach(function(){
    this.template = "<div class='house-'>" +
                      "<span class='name-'></span>" +
                    "</div>";
  });
  it("should be set in the html", function () {
    var model = new Backbone.Model({house: {name: 'Peacock'}})
      , template = generateTemplate(model, this.template);
    expect($(".name-").html()).to.be("Peacock");
  });
});
